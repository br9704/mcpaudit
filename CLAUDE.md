...
Shipped: 17 checks (9 conformance C0–C8, 7 safety S1–S7, plus D1 drift), both transports, era detection, terminal/JSON/SARIF reports, pin+drift, **114 tests green**, lint + typecheck clean, zero runtime deps (enforced by test), RULES.md generated from rule metadata with a staleness test, README with a real findings table over 4 audited reference servers, SECURITY.md, CONTRIBUTING.md, issue templates, CI on Node 20/22/24 + weekly spec-drift job. Clean-machine `npm pack` → `npx` verified.
...
**Remaining (owner only):** trusted-publisher configuration on npm, so tagged releases from `v0.1.1` carry provenance (web UI only — npm 11.6.2 has no CLI surface for it); the optional `mcp-audit` dispute ticket. Nothing to disclose — no exploitable finding was made. *(Publish, repo-public flip and badges are DONE as of 2026-08-15; see the current-state line above.)*

**brunojaamaa.dev cross-link — not done, and deliberately so.** mcpaudit is already staged in the portfolio at `lib/projects.ts` (`stagedProjects`), and that repo carries a written 6-step promotion procedure whose only gate is "a real npm page" — now satisfied. It was **not** executed because the portfolio working tree holds 57 uncommitted files from another pass, including `lib/projects.ts`, `public/llms.txt` and 30 SEO baselines — precisely the files promotion touches. Do it when that tree is clean. Note for whoever does: every portfolio doc still says `@br9704/mcp-audit`, and `docs/CLAIMS-REGISTER.md` asserts mcpaudit 404s on npm; both are now false.
